...
[![Version](https://img.shields.io/badge/version-1.0.74-blue.svg)](https://github.com/jerelryoshida-dot/cart-quote-woocommerce-email)
...
### 1.0.74
- Feature: Mini-cart dropdown refresh now matches PHP template layout
- Added: Parent+tier grouping in JavaScript refresh
- Added: `parent-item` and `tier-item` CSS classes
- Added: Separator lines between product groups
- Added: Respects `show_tier_items` Elementor setting via `data-show-tier-items` attribute
- Added: Aggregates quantities and prices for parent items
...
- Reverted: HTML classes back to `.cart-quote-mini-cart-wrapper` and `.cart-quote-mini-count`
- Result: Visual appearance restored (red badge, etc.)
- Note: JavaScript still supports both selectors for backward compatibility
...
- Improved: Cross-plugin compatibility with WCGM's field naming conventions

### 1.0.56
- Debug: Added visible data dump to Elementor mini-cart widget dropdown
- Previous debug (v1.0.55) only worked in shortcode template, not Elementor widget
- Shows: product_id, tier_data, selected_tier, _all_tiers in green on black
